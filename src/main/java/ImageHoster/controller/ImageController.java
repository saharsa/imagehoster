...
public class ImageController {
  ...
  /**
   * This controller method is mapped to /images/{imageId}/{title} path. It fetches the image from
   * the database corresponding to the id. It receives the dynamic parameter imageId and title from
   * incoming request URL. Calls the getImage(imageId) method of ImageService. Add the image in the
   * Model type object with 'image' as the key Return 'images/image.html' file Also now you need to
   * add the tags of an image in the Model type object
   *
  ...
  public String editImage(@RequestParam("imageId") Integer imageId, Model model,
      HttpSession session) {
    // If no image id is provided for editing, then redirecting to images
    if (imageId == null) {
      return "redirect:/images";
    }
    Image image = imageService.getImage(imageId);
    // If image does not exist, then redirecting to images
    if (image == null) {
      return "redirect:/images";
    }
